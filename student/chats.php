...
<div class="main">
 ...
 <div data-aos="fade-up" class="row" style="margin-top: 30px; margin-bottom: 50px;">
    ...
    <div data-aos="fade-up" data-aos-duration="1000"  class="col-9 flex" style="margin-left: 70px;">
      ...
      <p>Chats history from instructors will appear below.</p>
      <?php
      ...
      <div data-aos="fade-up" class="card bg-dark" style="margin-bottom: 100px; background-color: #23272a; width: 70%; margin:auto;">
        <div class="card-title text-center">
          ...
          <form class="form-inline mt-4" style = "display : inline-block" method = "POST" action = "search-users.php">
            <input class="form-control mr-sm-2" type="search" name = "search" placeholder="Search for instructor" aria-label="Search" style="margin-bottom: 20px;">
        ...
        <div class="card-body mb-4">
          <?php
            $lastMessage = "SELECT DISTINCT received_by FROM messages WHERE sent_by = '$useremail'";
            ...
            if(mysqli_num_rows($lastMessageStatus) > 0) {
              while($lastMessageRow = mysqli_fetch_assoc($lastMessageStatus)) {
                $sent_by = $lastMessageRow['received_by'];
                // changeable user
          ...
          <div class="card text-white" style="background-color: #2c2f33;">
            <div class="card-body">
              <h6><strong><?=$lastMessageRow['received_by'];?></strong><a href="./message.php?receiver=<?=$sent_by?>" class="btn btn-outline-warning" style = "float:right">Send message</a></h6>
            </div>
          </div><br/>
          <?php
            }
